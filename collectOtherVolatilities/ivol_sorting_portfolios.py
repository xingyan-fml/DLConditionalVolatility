...
def make_table(portfolios, model_name, sorting, weight, numGroups):
    idx = pd.IndexSlice
    with open("ivol_portfolio_" + weight + ".txt", 'a') as file:
...
with open("ivol_portfolio_ew.txt", 'w') as file:
    file.write('model_name,sorting,group,Pred,Real,Std,SR\n')
with open("ivol_portfolio_vw.txt", 'w') as file:
    file.write('model_name,sorting,group,Pred,Real,Std,SR\n')
...
for i, model in enumerate(Models):
    ...
    usa = pd.read_csv('../preprocessing/usa_new.csv')[['eom', 'id', 'ivol_capm_252d']]
    usa.columns = ['date', 'id', 'ivol_capm_252d']
    data = pd.merge(data, usa, how = 'left', on = ['date', 'id'])
    data['std'] = data['ivol_capm_252d']
