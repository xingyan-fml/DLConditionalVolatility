...
for filename in os.listdir(folder_path):
    if filename.endswith('.csv') and 'new' not in filename:

        file_path = os.path.join(folder_path, filename)
        data = pd.read_csv(file_path)
        data = data[info+cols]

        # Drop all observations with stock price < 1 USD:
        data = data[data["prc"] >= 1]
        # Drop all observations where the market capitalisation < 5mio USD:
        data = data[data["me"] >= 5]

        data.dropna(how='any',subset=["ret_exc_lead1m"],inplace=True)

        pd.set_option('display.max_rows',999)
        print(data.isna().sum().sort_values(ascending=False))

        data = data.fillna(data.groupby('eom').transform('median'))
        data.dropna(inplace=True)
        print(data.isna().sum().max())

        data = data[(data['eom']>=19910131)&(data['eom']<=20201231)]
        print('number of months: {}'.format(data['eom'].nunique()))
        print('number of stocks: {}'.format(data['id'].nunique()))

        data.sort_values(by = ['eom','id'], inplace = True)

        data.to_csv(filename[:-4] + "_new.csv", index = False)