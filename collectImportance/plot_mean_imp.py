...
mean_imp = pd.read_csv('mean_imp.csv')
...
mean_imp['average'] = mean_imp.drop('char', axis = 1).mean(axis = 1) * 100
mean_imp_sorted = mean_imp.sort_values(['average', 'char'], ascending = [True, False]).iloc[-20:]

pdf = PdfPages('plots/mean_importance.pdf')
mean_imp_sorted.plot(kind='barh', x='char', y='average', figsize = (8, 6), width = 0.8)
...
for i, hidden_dims in enumerate(Hidden_dims):
    importance_all[str(hidden_dims)] = mean_imp[str(hidden_dims)].rank(pct = False)

importance_all.index = mean_imp['char']
...
pdf = PdfPages('plots/mean_importance_all.pdf')
...
conditions = [mean_imp['char'].isin(char_cate) for char_cate in char_category]
mean_imp['category'] = np.select(conditions, category, default = np.nan)
importance_cate = mean_imp.groupby(['category'])[[str(x) for x in Hidden_dims]].mean()
...
pdf = PdfPages('plots/mean_importance_category.pdf')
