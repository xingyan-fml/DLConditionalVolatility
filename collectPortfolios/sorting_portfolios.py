...
def make_table(portfolios, model_name, sorting, weight, numGroups):
    idx = pd.IndexSlice
    with open("portfolio_" + weight + ".txt", 'a') as file:
...
with open("portfolio_ew.txt", 'w') as file:
    file.write('model_name,sorting,group,Pred,Real,Std,SR\n')
with open("portfolio_vw.txt", 'w') as file:
    file.write('model_name,sorting,group,Pred,Real,Std,SR\n')
...
for weight in ['ew', 'vw']:
    results = pd.read_csv("portfolio_" + weight + ".txt").set_index(['model_name', 'sorting', 'group'])
    with open("portfolio_" + weight + "_latex.txt", 'w') as file:
        for model_name in Model_names:
            file.write("& \\multicolumn{{4}}{{c}}{{{} Single}}".format(model_name))
            file.write(" & \\multicolumn{{4}}{{c}}{{{} Independent Double}}".format(model_name))
            file.write(" & \\multicolumn{{4}}{{c}}{{{} Dependent Double}} \\\\\n".format(model_name))
            file.write("& Pred & Real & Std & SR & Pred & Real & Std & SR & Pred & Real & Std & SR \\\\\n")
            file.write("\\midrule\n")
            for group in ['Low(L)'] + [str(x+1) for x in range(1, numGroups - 1)] + ['High(H)', 'H-L']:
                file.write(group + ' ')
                for sorting in ['univariate', 'independent', 'dependent']:
                    result = results.loc[(model_name, sorting, group)]
                    file.write(f'& {result[0]:.2f} & {result[1]:.2f} & {result[2]:.2f} & {result[3]:.2f} ')
                file.write('\\\\\n')
            file.write("\\midrule\n")
