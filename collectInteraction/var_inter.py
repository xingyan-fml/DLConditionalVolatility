...
Marginal_chars = ['ivol_capm_252d', 'prc', 'rvol_21d', 'prc_highprc_252d']
...
for marginal_char in Marginal_chars:
    ...
    for i in range(20):
        ...
        for n in range(nn_Num):
            model_temp  = variance_nn(data.shape[1], hidden_dims)
            model_temp.load_state_dict(torch.load(f'{model_folder}/model_var_{hidden_dims}_{n}.pth'))
            model_list.append(model_temp)
        
        with torch.no_grad():
            var_ensem = np.zeros((len(data), nn_Num))
            for n in range(nn_Num):
                var_ensem[:, n] = model_list[n](data_x).detach().numpy().reshape(-1)
        pred[:, i] = var_ensem.mean(axis = 1)
    
    results[str(hidden_dims)] = pred.mean(axis = 1)
    results = results.sort_values(by=marginal_char).reset_index(drop=True)
    
    pdf = PdfPages(f'plots/{marginal_char}_var.pdf')
    plt.figure(figsize=(7,5),dpi=300)
    for i in [-1, -0.5, 0, 0.5, 1]:
        results_temp = results[results[main_char] == i]
        plt.plot(results_temp[marginal_char], np.sqrt(results_temp[str(hidden_dims)]), label = f'{main_char} = ${i}$')
